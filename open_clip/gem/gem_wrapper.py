...
from .gem_utils import SelfSelfAttention, VanillaAttention, GEMResidualBlock, modified_vit_forward
...
class GEMWrapper(nn.Module):
    ...
    def apply_gem(self):
        for i in range(0, len(self.model.visual.transformer.resblocks)):
            ...
            qkv_bias = True
            if i > len(self.model.visual.transformer.resblocks) - self.depth:
                ...
                ss_attn = VanillaAttention(dim=dim, num_heads=num_heads, qkv_bias=qkv_bias,
                                            ss_attn_iter=self.ss_attn_iter, ss_attn_temp=self.ss_attn_temp)
        ...
        self.model.visual.forward_all = modified_vit_forward.__get__(self.model.visual, VisionTransformer)
        return
