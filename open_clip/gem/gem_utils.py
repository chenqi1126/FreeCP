...
class SelfSelfAttention(nn.Module):
    ...
    def forward(self, x, attn_bias=None, prev_attn=None):
        ...
        return [x.transpose(0, 1), x_ori.transpose(0, 1), attn_ori.mean(0, keepdim=False)]
...
class VanillaAttention(nn.Module):
    ...
    def forward(self, x, attn_bias=None, prev_attn=None):
        ...
        return [x_ori.transpose(0, 1), attn_ori.mean(0, keepdim=False)]
...
class GEMResidualBlock(nn.Module):
    ...
    def forward(self,
                ...
                attn_weights: Optional[List[torch.Tensor]] = None
                ):
        if isinstance(q_x, list):
            if len(q_x) == 2:
                q_x, attn_weights = q_x
                x_gem = q_x
            elif len(q_x) == 3:
                x_gem, q_x, attn_weights = q_x
        # else:
        #     x_gem = q_x

        outputs = self.res_block.attn(x=self.res_block.ln_1(q_x))
        if len(outputs) == 2:
            x_ori_res, attn_weight = outputs
            x_ori_res = self.res_block.ls_1(x_ori_res)
        elif len(outputs) == 3:
            x_gem_res, x_ori_res, attn_weight = outputs
        ...
        if len(outputs) == 2:
            return [x_ori, attn_weights]
        return [x_gem, x_ori, attn_weights]

class GEMViT(nn.Module):
    def __init__(self, vit):
        self.vit = vit

def modified_vit_forward(self, x: torch.Tensor, model_type):
        ...
        x_gem, x, attn_weights = self.transformer(x)
        ...
        return x_gem, x, attn_weights

def custom_attn(attn_layer, x, attn_type='vanilla', ss_attn_temp=None, ss_attn_iter=1):
        ...
        attn_output_ori = attn_layer.out_proj(attn_output)

        if attn_type == 'gem':
            xs1, xs2, xs3 = v, k, q
            if ss_attn_temp is None:
                ## B, N, C
                pre_norm = torch.norm(x.transpose(0,1), dim=-1).mean(dim=-1, keepdim=True).unsqueeze(-1)
                inv_temp = pre_norm * scale
            else:
                inv_temp = ss_attn_temp

            for it in range(ss_attn_iter):
                xs1 = F.normalize(xs1, dim=-1)
                xs2 = F.normalize(xs2, dim=-1)
                xs3 = F.normalize(xs3, dim=-1)
            ...
            xs2 = attn2 @ v
            xs3 = attn3 @ v
            attn_output = (xs1 + xs2 + xs3) / 3

            attn_output = attn_output.transpose(0, 1).contiguous().view(-1, bsz, embed_dim)
            attn_output_gem = attn_layer.out_proj(attn_output)

            return  attn_output_ori, attn_output_gem, attn_weights.mean(0, keepdim=True)

        return attn_output_ori, attn_weights.mean(0, keepdim=True)