...
class VisionTransformer(nn.Module):
    ...
    def forward_all(self, x: torch.Tensor, model_type: str = 'ClearCLIP', last_n_layers=1):
        B, nc, w, h = x.shape
        ...
        for i, blk in enumerate(self.transformer.resblocks[:-last_n_layers]):
            attn_output, attn_weight = self.custom_attn(blk.attn, blk.ln_1(x), with_attn=True, model_type='vanilla')
            x = x + attn_output
        ...
        for blk in self.transformer.resblocks[-last_n_layers:]:
            attn_ori, attn_weight = self.custom_attn(blk.attn, blk.ln_1(x), with_attn=True, model_type='vanilla')
            attn_mod = self.custom_attn(blk.attn, blk.ln_1(x), with_attn=False, model_type=model_type)  
            x_mod += attn_mod
            if not model_type == 'ClearCLIP':
                x_mod = x_mod + blk.mlp(blk.ln_2(x_mod))
            x_ori = x + attn_ori
            x_ori = x_ori + blk.mlp(blk.ln_2(x_ori))
            attn_weights.append(attn_weight)

        x_mod = x_mod.permute(1, 0, 2)  # LND -> NLD
        x_ori = x_ori.permute(1, 0, 2)  # LND -> NLD
        return self.ln_post(x_mod) @ self.proj, self.ln_post(x_ori) @ self.proj, attn_weights
    ...
    def custom_attn(self, attn_layer, x, model_type='ClearCLIP', with_attn=False):
        ...
        if with_attn:
            return attn_output, attn_weights.mean(0, keepdim=True)
